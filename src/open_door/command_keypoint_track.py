...
class KeyPointTrack(Command):
    ...
    def __init__(
        ...
        with torch.device(self.device):
            ...
            # Hand end-effector target buffers (body frame, relative to torso)
            # target_hand_pos_b: (N, 2, 3) — left, right hand target in body frame
    ...
    def _update_hand_targets(self):
        """Compute hand position errors and resample targets periodically."""
        ...
        # Resample hand targets on interval
        interval_reached = (self.env.episode_length_buf - 20) % self.hand_resample_interval == 0
        resample_hand = interval_reached & self.with_prob(self.num_envs, self.resample_prob)
        resample_ids = resample_hand.nonzero().squeeze(-1)
    ...
    def _init_hand_targets(self, env_ids: torch.Tensor):
        """Set hand targets to current hand positions so error starts at 0."""
        ...
        self.target_hand_pos_b[env_ids, 0] = hand_rel_b_left
        self.target_hand_pos_b[env_ids, 1] = hand_rel_b_right
        self.hand_pos_error_b[env_ids] = 0.0

    def sample_hand_command(self, env_ids: torch.Tensor):
        """Sample random target positions for both hands in body frame relative to torso."""
        if len(env_ids) == 0:
            return
        n = len(env_ids)
        # Sample random positions in the target range for each hand
        for i in range(2):  # 0=left, 1=right
            ...
            self.target_hand_pos_b[env_ids, i] = target_b
    ...
    def debug_draw(self):
        ...
        quat_w = self.asset.data.root_link_quat_w
        yaw_q = yaw_quat(quat_w)
        # target_hand_pos_b: (N, 2, 3) -> rotate each to world and add torso offset
        ...
        if self.env.backend == "isaac":
            ...
            # Draw hand targets (red) and current hand positions (green)
